...
func InitDB() *sql.DB {
	db, err := sql.Open("mysql", "root:1818@tcp(127.0.0.1:3306)/task_manager")
	if err != nil {
...
func handleTasks(db *sql.DB, w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodPost:
		...
		if task.Description == "" { // Check if task description is empty
...
func addTasksToDatabase(db *sql.DB) {
	...
	tasks := []Task{
		...
		{Description: "Task 31", Status: "Started"},
